// For this exercise we'll be creating a raw TCP server. There's no HTTP involved
//here so we need to use the net module from Node core which has all the basic networking functions.
//
// The net module has a method named net.createServer() that takes a callback function. Unlike
//most callbacks in Node, the callback used by createServer() is called more than once.
...
//
// The socket object contains a lot of meta-data regarding the connection, but it
//is also a Node duplex Stream, in that it can be both read from, and written to.
...
//socket. Alternatively, the .end() method also takes a data object so you can simplify to
...
//
// To create the date, you'll need to create a custom format from a new Date() object.
//The methods that will be useful are:
//
//     date.getFullYear()
